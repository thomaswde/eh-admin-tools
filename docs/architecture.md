...
The browser is served by a loopback-only FastAPI application and calls only local `/backend` routes. `main.py` validates local requests and workspace cookies; `backend/session_store.py` owns bounded, expiring workspace entries with an optional authenticated client; `backend/extrahop_client.py` owns RevealX 360 OAuth, Enterprise API-key authentication, TLS policy, connection reuse, upstream retries, and ExtraHop requests. Credentials stay server-side and saved credentials use the operating-system credential service. Browser code must not call ExtraHop hosts directly or persist credentials.
