...
Packetstore metric sources are identified independently from the Packet Sensor metric set. Inventory-confirmed all-in-one systems use their shared system ID, and standalone Packetstores use the Trace appliance's own system ID; both remain eligible when lookback is zero or unavailable. A positive Packetstore lookback may confirm integrated capability on an otherwise ordinary sensor as a compatibility fallback. A zero-only `cpc` probe is indeterminate, not detected, and must not promote generic interface-drop counters into Packetstore rows or findings. The same source collection drives requests, coverage, report rows, and exports so measurements remain attributed to the appliance that produced them.
