...
Packetstore metric sources are identified independently from the Packet Sensor metric set without speculative Metrics API calls. Standalone Packetstores use the Trace appliance's own system ID. All-in-one systems use their shared system ID when confirmed by the product catalog, the `eda_onboard_trace` licensed feature, or the normalized `EDA...70[V]` model-name convention as a catalog-lag fallback. Standalone `...50[V]` model matching similarly protects incomplete inventory metadata. Ordinary Packet Sensors are never promoted by a `cpc` response. Inventory-confirmed sources remain eligible when lookback is zero or unavailable, and the same source collection drives requests, coverage, report rows, and exports so measurements remain attributed to the appliance that produced them.
